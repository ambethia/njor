...
module BattleNet
  def self.access_token
    ...
    unless client_credentials && Time.parse(client_credentials["expires_at"]).future?
      ...
      data = JSON.parse(request.post("/oauth/token", "grant_type=client_credentials").body)
  ...
  def self.get(path_or_uri, namespace = "static-us")
    uri = URI(path_or_uri)
    request = Faraday.new("https://us.api.blizzard.com") do |conn|
      conn.authorization :Bearer, access_token
    end
    JSON.parse(request.get(uri.path, namespace: namespace, locale: "en_US").body)
  end
end
